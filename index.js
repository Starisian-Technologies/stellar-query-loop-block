...
registerBlockType("sparxstar/user-post-query", {
  ...
  edit({ attributes, setAttributes }) {
    ...
    const [statusOptions, setStatusOptions] = useState([]);
    useEffect(() => {
      if (window.SparxStarPostStatusOptions) {
        setStatusOptions(window.SparxStarPostStatusOptions);
      }
    }, []);
      ...
      <>
        <InspectorControls>
          <PanelBody title="Query Settings">
            ...
            <CheckboxControl
              label="Post Status"
              help="Select one or more post statuses."
              checked={false} // Just to show the group label
              onChange={() => {}} // No-op
            />
            {statusOptions.map((status) => (
              <CheckboxControl
                ...
                onChange={(isChecked) => {
                  const newStatuses = isChecked
                    ? [...attributes.postStatus, status.value]
                    : attributes.postStatus.filter((s) => s !== status.value);
                  setAttributes({ postStatus: newStatuses });
                }}
              />
            ))}
            <SelectControl
            ...
            <TextControl
              ...
              value={attributes.postsPerPage}
              onChange={(val) =>
                setAttributes({ postsPerPage: parseInt(val, 10) || 1 })
              }
            />
            <SelectControl
              label="Order By"
        ...
        <div {...blockProps}>
          <ServerSideRender
            block="sparxstar/user-post-query"
